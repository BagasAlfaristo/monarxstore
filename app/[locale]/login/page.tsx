...
const dict = {
  en: {
    ...
    // page
    title: "Sign in to Monarx AI Store",
    subtitle:
      "Use your email and password to access your dashboard and orders.",
    emailLabel: "Email",
  ...
  zh: {
    ...
    // page
    title: "登录 Monarx AI Store",
    subtitle: "使用邮箱和密码登录，以查看订单和管理账号。",
    emailLabel: "邮箱",
    passwordLabel: "密码",
    buttonLabel: "登录",
    noAccount: "还没有帐号？",
...
export default async function LoginPage({
  ...
  const makeUrl = (overrides: MakeUrlOverrides = {}) => {
    ...
    const section = overrides.section ?? "login";
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      {/* HEADER – sama pattern dengan home/products */}
        ...
        <div className="mx-auto flex max-w-6xl items-center gap-4 px-4 py-3">
          ...
          <div className="hidden items-center gap-3 md:flex">
            ...
            {currentUser ? (
              ...
              <>
                {/* di halaman login sendiri, tombol ini cuma buat konsisten layout */}
                <span className="inline-flex w-20 items-center justify-center rounded-full border border-slate-200 px-4 py-1.5 text-xs font-medium text-slate-400">
                  {t.navSignIn}
                </span>
                <Link
      ...
      <div className="mx-auto flex max-w-6xl justify-center px-4 pb-10 pt-8">
        <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white px-6 py-7 shadow-sm">
          ...
          <form action="/api/auth/login" method="POST" className="mt-5 space-y-4 text-xs">
            {redirect && (
          ...
          <p className="mt-4 text-[11px] text-slate-600">
            ...
            <Link
              href={
                redirect
                  ? `${makeUrl({ section: "register" })}&redirect=${encodeURIComponent(
                    redirect
                  )}`
                  : makeUrl({ section: "register" })
              }
